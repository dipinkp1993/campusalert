...
if(mysqli_num_rows($result) > 0) {
    ...
    $query = "insert into login values(null,'$email','$password','parent')";
    $result = mysqli_query($pc, $query);
    if($result > 0) {
        ...
        $sid=$roww[0];
         ...
         $query = "insert into parent(login_id,pname,sturegno,phone,dep,sem) values($login_id,'$name','$username','$phone','$did','$sid')";
        //  echo $query;
        $result = mysqli_query($pc, $query);
        if($result > 0) {
                $data = array(
